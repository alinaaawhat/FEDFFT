...
def load_llm_dataset(config=None, **kwargs):
    ...
    if dataset_name.endswith('.json'):
    ...
    elif dataset_name.lower() == 'gsm8k_diver':
        # Define the path for saving the dataset
        fp = os.path.join(config.data.root, 'gretel_gsm8k_train.jsonl')
        
        if not os.path.exists(fp):
            # Using huggingface_hub to download the dataset from Hugging Face
            from datasets import load_dataset
            
            # Load the dataset from Hugging Face
            dataset = load_dataset("gretelai/gretel-math-gsm8k-v1", split="train")
            
            # Convert to the format needed and save as jsonl
            with open(fp, 'w') as f:
                for item in dataset:
                    entry = {
        ...
        # Load the saved jsonl file
        list_data_dict = load_jsonl(fp,
                                instruction='instruction',
                                output='output',
                                category='category')
        
        # Format the output similar to the original code
        for i in range(len(list_data_dict)):
            # Check if "####" exists in the output and replace it
            if "####" in list_data_dict[i]['output']:
                list_data_dict[i]['output'] = \
                    list_data_dict[i]['output'].replace('####', 'The answer is')
        
        # Create the dataset using the formatted data
        dataset = LLMDataset(list_data_dict, tokenizer)
