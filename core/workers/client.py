...
class Client(BaseClient):
    ...
    def __init__(self,
        ...
        #------------------------My input----------------------------
        adapter_args = self._cfg.get('llm', {}).get('adapter', {}).get('args', [])
        if isinstance(adapter_args, list) and adapter_args:
            if all(arg.get('adapter_method') == 'fft' for arg in adapter_args):
                print("----------FFT methods and add the local position(client:149)-----------")
                if self._cfg['model']['type'].startswith("gpt"):
                    new_local_position = self._model.model.model.transformer.h[0].mlp.c_proj.indices['default']
                elif self._cfg['model']['type'].startswith("meta-llama") or self._cfg['model']['type'].startswith("Qwen"):
                    new_local_position = self._model.model.base_model.model.model.layers[0].self_attn.q_proj.indices['default']

                new_random_loc_seed = self._model.model.active_peft_config.random_loc_seed
                self.posit = new_local_position
                self.random_loc_seed = new_random_loc_seed
        #--------------------------------------------------------------------
    ...
    def callback_funcs_for_model_para(self, message: Message):
        ...
        if 'ss' in message.msg_type:
            ...
            # and synchronized with the received mode
            # print("-----------------------------------------------------------------")
            # if str(self.model.model.base_model).startswith("FourierFT"):
            #     aggregated_indices = []            
            #     layer = self.model.model.base_model.model.model.layers[0]
            #     b_matrix = layer.self_attn.q_proj.indices['default']
            #     avg_position = message.content['posit']
            #     aggregated_indices.append(b_matrix)
            #     clients_posit = torch.cat(aggregated_indices, dim=0)
            #     print('fft, client.py callback_funs_for_model_para')
            #     print(f"clients posit + {clients_posit}")
            #     #create content2 (match the size of local model)
            #     # content2 = {key: [] for key in content}
            #     content2 = OrderedDict()
            #     for key in content.keys():
            #         content2[key] = []
            #     for i, client_pos in enumerate(clients_posit):
            #         for j, avg_pos in enumerate(avg_position):
            #             if torch.equal(client_pos, avg_pos):  
            #                 for key in content:
            #                     content2[key].append(content[key][j])   
            #     content = content2
            ...
            if self.is_unseen_client or skip_train_isolated_or_global_mode:
                ...
                if hasattr(self, "posit") and self.posit is not None:
                    model_para_all['posit'] = self.posit
                ...
                #-------------------------------------------
                # write in file
                # print(type(train_log_res))  #type 
                # print(train_log_res)  
                log_filename = "train_log_output.txt"
                with open(log_filename, "a") as f:
                    for key, value in train_log_res.items():
                        f.write(f"{key}: {value}\n")
                #---------------------------------------------------------
                #logger.info(train_log_res)
