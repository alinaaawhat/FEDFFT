...
class Server(BaseServer):
            ...
            if not check_eval_result:
                ...
                if self.state < self.total_round_num:
                    ...
                    logger.info('Server: Training is finished! Starting '
                                'evaluation.')
                    # 合并适配器权重到基础模型
                    # merged_layer = self.model.model.base_model.model.transformer.h[0].mlp.c_proj.merge()
                    merged_model = self.model.model.merge_and_unload() 
                     # 保存合并后的模型 
                    output_dir = f"./trained_model/{self._cfg['expname']}" 
                    merged_model.save_pretrained(output_dir)
                    self.eval()
                    # if self._cfg['data'].type.startswith("dolly-15k"):
                    #     self.evaluate_on_alpaca(merged_model)
                    # elif self._cfg['data'].type.startswith("gsm"):
                    #     self.evaluate_on_gsm8k(model_name = merged_model, device = self._cfg.device)
    ...
    def _perform_federated_aggregation(self):
        ...
        for model_idx in range(self.model_num):
            ...
            # #1.判断是否是“Fourierft”
            # #-----------------------Extend the model size---------------------------
            # first_key = next(iter(merged_param.keys()), '')
            # extend_model_size = merged_param[first_key].shape[0]
            # # 1.判断是不是fft
            # if ('fourierft' in first_key and
            #     merged_param[first_key].shape[0] != self.model.model.peft_config['default'].n_frequency):
            # # 1. 首先更新模型配置中的 n_frequency
            #     self.model.model.peft_config['default'].n_frequency = extend_model_size   
            #     # 2. 遍历模型中的所有 FourierFTLayer，更新其 posit
            # #------------------------------Method1--------------------------------------
            #     for name, module in self.model.named_modules():
            #         if isinstance(module, FourierFTLinear):
            #             # Check if this module has fourierft_spectrum attribute
            #             if hasattr(module, "fourierft_spectrum"):
            #                 big_size_position = extend_model_size
            #                 # Update posit
                            
            #                 module.update_layer(
            #                     adapter_name = 'default',
            #                     n_frequency=big_size_position,
            #                     scaling = self.model.model.peft_config['default'].scaling,
            #                     # Use default values if peft_config is not available
            #                     init_weights=getattr(self, "peft_config", {}).get("default", {}).get("init_weights", None),
            #                     random_loc_seed = module.fourierft_random_loc_seed['default']
            #                 )
                            
            #                 print(f"Updated {name} to-------------position is {module.indices['default']}")
            #                 module.indices['default'] = merged_param['posit']
            #                 # 去掉"posit"
            #     merged_param = {k: v for k, v in merged_param.items() if 'posit' not in k}
            # #-----------------------------------------
