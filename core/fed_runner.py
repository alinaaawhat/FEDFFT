...
class BaseRunner(object):
    ...
    def _setup_client(self,
        ...
        #--Method 3 change position and indices--------------------------------------------------------------------------
        # if str(client_model.model.base_model).startswith("FourierFTModel"):
        ...
        #             **kw)
        #     layer = client.model.model.base_model.model.model.layers[0]
        #     b_posit = layer.self_attn.q_proj.indices['default']
        #     #     b_matrix = layer.self_attn.q_proj.indices['default']
        #     #     aggregated_indices.append(b_matrix)
        #     random_loc_seed = client.model.model.base_model.peft_config['default'].random_loc_seed
        #     print(f"Client {client_id} .........random_loc_seed is {random_loc_seed} position {b_posit}")
        # else:
        ...
        if client_id == -1:
            ...
            # print(client.model.model.peft_config['default'].random_loc_seed)
...
class StandaloneRunner(BaseRunner):
    def _set_up(self):
        ...
        for client_id in range(1, self.cfg.federate.client_num + 1):
            #Method 1 create the new model
            # if str(self._shared_client_model.model.base_model).startswith("FourierFT"):
            #     server_data = self.data[self.server_id]
            #     self._shared_client_model = get_model(
            #     self.cfg, self.server_data, backend=self.cfg.backend
            #     )
            # self._shared_client_model.model.model.peft_config['default'].random_loc_seed = random.randint(0,100)
            # indi_seed = self._shared_client_model.model.model.peft_config['default'].random_loc_seed
            # print(f"client[{client_id}]     random_loc_seed is {indi_seed}")

            # Method 2 update_layer method
            # Update_layer's parameter setting and print the previous position
            
            # if self.cfg.llm.adapter.args[0]['adapter_method'] == 'fft':
            #     # random_loc_seed = random.randint(100,200)
            #     adapter_name = "default" # self.cfg.llm.adapter.args[0]['adapter_package']
            #     n_frequency = self.cfg.llm.adapter.args[0]['n_frequency']
            #     scaling = self.cfg.llm.adapter.args[0]['scaling']
            #     init_weights = False
            #     # old_local_position = self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.indices['default']
            #     # old_random_loc_seed = self._shared_client_model.model.model.peft_config['default'].random_loc_seed
            #     # print(f"previous client[{client_id}] without change    random_loc_seed is {old_random_loc_seed} position {old_local_position}")
            #     # #Update_layers
            #     random_loc_seed = random.randint(10,200) 
            #     self._shared_client_model.model.peft_config['default'].random_loc_seed = random_loc_seed
            #     self._shared_client_model.model.active_peft_config.random_loc_seed = random_loc_seed
                #-------------------------------------------test--------------------------------------------
                # scaling = 0
                # self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.update_layer(adapter_name, n_frequency, scaling, init_weights, random_loc_seed)
                # new_local_position = self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.indices['default']
                # print(f"------------------你看看这个位置是啥seed=0---------------------{new_local_position}")
                # random_loc_seed = 1 
                
                # self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.update_layer(adapter_name, n_frequency, scaling, init_weights, random_loc_seed)
                # new_local_position = self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.indices['default']
                # print(f"------------------你看看这个位置是啥seed = 1-----------------------{new_local_position}")
#----------------------------------------------------------------------------------------------------------------
                # #gpt
                # if self.cfg['model']['type'].startswith("gpt"):
                #     for i in range(12):  
                #         # Update the attention projections
                #         # In GPT2, the output projection of the MLP
                #         self._shared_client_model.model.model.transformer.h[i].mlp.c_proj.update_layer(
                #             adapter_name, n_frequency, scaling, init_weights, random_loc_seed)

                #     # Get the updated position and random location seed for logging
                #     new_local_position = self._shared_client_model.model.model.transformer.h[0].mlp.c_proj.indices['default']
                #     new_random_loc_seed = self._shared_client_model.model.model.transformer.h[0].mlp.c_proj.fourierft_random_loc_seed

                #     print(f"updated client[{client_id}] changing..........0_random_loc_seed is {new_random_loc_seed} position {new_local_position}")
                
                # #qwen
                # elif self.cfg['model']['type'].startswith('Qwen'):
                #     for i in range(28):
                #         self._shared_client_model.model.model.model.layers[i].self_attn.q_proj.update_layer(adapter_name, n_frequency, scaling, init_weights, random_loc_seed)
                #         new_q_local_position = self._shared_client_model.model.model.model.layers[i].self_attn.q_proj.indices['default']
                #         q_random_loc_seed = self._shared_client_model.model.model.model.layers[i].self_attn.q_proj.fourierft_random_loc_seed
                #         print(f"updated client[{client_id}] changing..........{i}q__random_loc_seed is {q_random_loc_seed} q_position {new_q_local_position}")

                #     # new_local_position = self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.indices['default']
                #     # new_random_loc_seed = self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.fourierft_random_loc_seed # self._shared_client_model.model.model.peft_config['default'].random_loc_seed
                #     # print(f"updated client[{client_id}] changing..........0_random_loc_seed is {new_random_loc_seed} position {new_local_position}")
                
                #  #llama
                # elif self.cfg['model']['type'].startswith("meta-llama"):
                #     for i in range(32):
                #         self._shared_client_model.model.model.model.layers[i].self_attn.q_proj.update_layer(adapter_name, n_frequency, scaling, init_weights, random_loc_seed)
                #         self._shared_client_model.model.model.model.layers[i].self_attn.v_proj.update_layer(adapter_name, n_frequency, scaling, init_weights, random_loc_seed)
                #     new_local_position = self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.indices['default']
                #     new_random_loc_seed = self._shared_client_model.model.model.model.layers[0].self_attn.q_proj.fourierft_random_loc_seed # self._shared_client_model.model.model.peft_config['default'].random_loc_seed
                #     print(f"updated client[{client_id}] changing..........0_random_loc_seed is {new_random_loc_seed} position {new_local_position}")    
